...
GITHUB_API_URL = "https://api.github.com/repos/manticoresoftware/manticoresearch/contents/manual/english"
DOCS_BASE_URL = "https://raw.githubusercontent.com/manticoresoftware/manticoresearch/master/manual/english/"

# Cache for documentation files list and content
_docs_cache: Optional[List[str]] = None
...
async def list_documentation_files() -> List[str]:
    ...
    Returns:
        List of documentation file paths (e.g., ["Searching/KNN.md", "Creating_a_table/Data_types.md"])
    """
    global _docs_cache
                        ...
                        if subitem['type'] == 'file' and subitem['name'].endswith('.md'):
                            ...
                            if subsubdir_response.status_code == 200:
                                ...
                                for subsubitem in subsubdir_items:
                                    if subsubitem['type'] == 'file' and subsubitem['name'].endswith('.md'):
                                        files.append(f"{item['name']}/{subitem['name']}/{subsubitem['name']}")
...
async def fetch_documentation(file_path: str, content: Optional[str] = None, 
